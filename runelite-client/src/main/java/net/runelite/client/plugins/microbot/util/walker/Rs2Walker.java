...
public class Rs2Walker {
    ...
    public static boolean walkToAndInteract(WorldPoint target, Predicate<GameObject> objectFilter, int distance) {
            ...
            try {
                while (!Thread.currentThread().isInterrupted()) {
                    ...
                    for (int i = indexOfStartPoint; i < path.size(); i++) {
                        ...
                        if(Rs2Player.getWorldLocation().distanceTo(target) < (distance * 2)) {
                            ...
                            if(object != null) {
                                ...
                                if(Rs2GameObject.interact(object)) {
                                    Microbot.log("Interact with object for walking early exit.");
                                    isFinished.set(true);
                                    return true;
                                }
                            }
        ...
        return Microbot.getClientThread().runOnSeparateThread2(walkingCallable);
    }
